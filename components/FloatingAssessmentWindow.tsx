...
const ReviewSessionView: React.FC<SessionProps> = ({ review, questions, mode, onComplete, onExit }) => {
    ...
    const theoryQuestions = allQuestions.filter(q => !q.text.startsWith('[PRACTICAL]'));
    const practicalQuestionsList = allQuestions.filter(q => q.text.startsWith('[PRACTICAL]'));
    ...
    const getSaved = (key: string, def: any) => {
        ...
        if (review.session_data) {
            ...
            if (key === 'pracChoice1' && sd.pracChoice1 !== undefined) return sd.pracChoice1;
            if (key === 'pracChoice2' && sd.pracChoice2 !== undefined) return sd.pracChoice2;
            if (key === 'pracChoice3' && sd.pracChoice3 !== undefined) return sd.pracChoice3;
    ...
    const [pracChoice1, setPracChoice1] = useState<number>(getSaved('pracChoice1', 0));
    const [pracChoice2, setPracChoice2] = useState<number>(getSaved('pracChoice2', 1));
    const [pracChoice3, setPracChoice3] = useState<number>(getSaved('pracChoice3', 2));
    ...
    const currentQuestion = theoryQuestions[currentIndex];
    ...
    }, [currentIndex, results, practicalMark, practicalMark2, practicalMark3, practicalLink, practicalLink2, practicalLink3, pracChoice1, pracChoice2, pracChoice3, seconds, notes, isPaused, review.id, review.status]);

    const stats = {
        theoretical: results.reduce((acc, curr) => acc + curr.score, 0),
        maxTheoretical: theoryQuestions.length * 10
    };

    const effectivePracticalMark = review.module === 'Module 4'
        ? (practicalMark + practicalMark2) / 2
        : practicalMark;

    const isPracticalScored = practicalQuestionsList.length > 0;

    const totalScore = isPracticalScored
        ? (stats.maxTheoretical > 0 ? (stats.theoretical / stats.maxTheoretical) * 70 : 0) + ((effectivePracticalMark / 10) * 30)
        : (stats.maxTheoretical > 0 ? (stats.theoretical / stats.maxTheoretical) * 100 : 0);
    ...
    const getReportText = () => {
        ...
        const pending = theoryQuestions.length - results.length;
...
-------------------------
${isPracticalScored ? (
                review.module === 'Module 4' ? `Project 1 (${practicalQuestionsList[pracChoice1]?.text?.replace('[PRACTICAL] ', '')}): ${practicalLink || 'N/A'} (Mark: ${practicalMark}/10)
Project 2: ${practicalLink2 || 'N/A'} (Mark: ${practicalMark2}/10)` :
                    `Practical (${practicalQuestionsList[pracChoice1]?.text?.replace('[PRACTICAL] ', '')}): ${review.module === 'Module 3' ? 'Live Session' : (practicalLink || 'N/A')} (Mark: ${practicalMark}/10)`
            ) : 'No practical tasks for this module.'}
        ...
        <div className="flex flex-col h-full bg-bg-subtle">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                <div className={cn(
                        ...
                        <>
                            <div className="flex justify-between items-center px-2">
                                <span className="badge bg-primary text-white border-none px-4 py-2 text-[10px] font-black uppercase tracking-widest">
                                    THEORY • Question {currentIndex + 1} of {theoryQuestions.length}
                                </span>
                                <div className="flex gap-2">
                                    ...
                                    <button className="btn btn-secondary w-10 h-10 p-0 shadow-sm" onClick={() => setCurrentIndex(Math.min(theoryQuestions.length - 1, currentIndex + 1))} disabled={currentIndex === theoryQuestions.length - 1}>
                                        <ArrowRight size={18} />
                                    </button>
                                </div>
                            </div>
                    ...
                    {/* Practical section if available */}
                    {!isMini && practicalQuestionsList.length > 0 && (
                        <div className={cn(
                            "pt-12 border-t border-border-base space-y-8",
                            isFullScreen ? "grid grid-cols-1 lg:grid-cols-2 gap-12 space-y-0" : ""
                        )}>
                            <div className="space-y-6">
                                <span className="badge bg-amber-500 text-white border-none px-4 py-2 text-[10px] font-black uppercase tracking-widest inline-block">
                                    Practical Assessments
                                </span>

                                <PracticalSlot
                                    label={review.module === 'Module 4' ? "Practical Project" : "Practical Task"}
                                    index={pracChoice1}
                                    setIndex={setPracChoice1}
                                    link={practicalLink}
                                    setLink={setPracticalLink}
                                    mark={practicalMark}
                                    setMark={setPracticalMark}
                                    list={practicalQuestionsList}
                                    hideLink={review.module === 'Module 3'}
                                />
                                ...
                                {review.module === 'Module 4' && (
                                    <>
                                        <PracticalSlot
                                            ...
                                            setIndex={() => { }}
                                            link={practicalLink2}
                                            setLink={setPracticalLink2}
                                            mark={practicalMark2}
                                            setMark={setPracticalMark2}
                                            list={[]}
                                            simpleMode
                                            borderT
                                        />
                            ...
                            <div className="space-y-4">
                                <label className="text-[10px] font-black uppercase text-text-tertiary tracking-widest px-1">Session Notes & Feedback</label>
                                <textarea
                                    className="w-full h-full min-h-[200px] p-6 bg-bg-white border border-border-base rounded-3xl text-base font-medium outline-none focus:border-primary focus:ring-4 focus:ring-primary-subtle transition-all resize-none shadow-sm placeholder:italic"
            ...
            <div className="p-6 bg-bg-white border-t border-border-base shrink-0">
                <button
                    ...
                    onClick={() => {
                        const needsLink = practicalQuestionsList.length > 0 && !isMini && review.module !== 'Module 3';
                        if (needsLink && !practicalLink) {
...
const PracticalSlot: React.FC<PracticalSlotProps> = ({ label, index, setIndex, link, setLink, mark, setMark, list, borderT, simpleMode, hideLink }) => (
    <div className={cn("space-y-6", borderT && "pt-6 border-t border-border-base/50")}>
        {!simpleMode && (
            <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-text-tertiary tracking-widest px-1">{label} Selection</label>
                <select
                    value={index}
                    onChange={(e) => setIndex(parseInt(e.target.value))}
                    className="h-12 w-full px-4 bg-bg-white rounded-xl text-sm font-bold border border-border-base focus:border-primary outline-none"
                >
                    {list.map((q, i) => (
                        <option key={q.id} value={i}>{q.text.replace('[PRACTICAL] ', '')}</option>
                    ))}
                </select>
                {list[index] && (
                    <div className="p-4 bg-primary-subtle rounded-xl border border-primary/10">
                        <div className="text-[10px] font-black uppercase text-primary mb-1 tracking-widest">Requirements</div>
                        <div className="text-xs font-medium text-text-secondary whitespace-pre-wrap">
                            {list[index].answer || 'No specific requirements mentioned.'}
                        </div>
                    </div>
                )}
            </div>
        ...
        {!hideLink && (
            <div className="space-y-4">
                <label className="text-[10px] font-black uppercase text-text-tertiary tracking-widest px-1">{label} URL</label>
                <div className="relative">
                    <input
                        type="text"
                        className="h-14 w-full px-6 bg-bg-white rounded-2xl text-base font-bold shadow-inner outline-none border border-border-base focus:border-primary transition-all"
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
                <label className="text-[10px] font-black uppercase text-text-tertiary tracking-widest">{label} Mark</label>
                <span className="text-xl font-black text-primary">{mark}/10</span>
            </div>
            <input
                type="range"
                min="0"
                ...
                className="w-full h-2 bg-bg-white rounded-lg appearance-none cursor-pointer accent-primary border border-border-base"
            />
        </div>
    </div>
);
